...
```
model-output/
├── CNCovidHub-ensemble/          # generated by the Hub, not submitted by a team
│   └── 2026-08-31-CNCovidHub-ensemble.csv
├── GZNL-NextWave/
...
`CNCovidHub-ensemble` is generated by the Hub itself and must **not** be submitted by a team. For each `(target, location, horizon)` combination it takes the weighted mean of the member models' values **at each quantile level** — a weighted Vincent average, the same aggregation [hubEnsembles](https://github.com/hubverse-org/hubEnsembles) performs by default. Averaging level by level keeps the resulting quantiles monotonic.

Membership and weighting rules:

- A model is eligible if its metadata has `designated_model: true` and it is not itself a Hub ensemble.
- Weights live in [`ensemble/weights.json`](../ensemble/weights.json) and are **relative**. They are renormalised over the models that actually contributed to each horizon, so a missing submission does not drag the ensemble down. Setting a model's weight to `0` excludes it while keeping a record of the decision.
- A model is dropped from a horizon if it did not supply all 23 required quantile levels for it.
- A horizon is only written when at least `min_models` (default 2) members contributed.

The ensemble is currently an **equal-weight mean**; weights will later be set from each model's WIS over a recent evaluation window.

Rebuild it with:

```bash
# latest configured round
python3 ensemble/build_ensemble.py

# a specific round, or every round, without writing anything
python3 ensemble/build_ensemble.py --reference-date 2026-08-31
python3 ensemble/build_ensemble.py --all-rounds --dry-run

# try a candidate weighting scheme before adopting it
python3 ensemble/build_ensemble.py --weights /tmp/wis-weights.json --dry-run
```

The `Build hub ensemble` workflow also runs it every Thursday 09:00 CST, after the submission deadline.
